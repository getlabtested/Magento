...
var CreditCardFieldsToggler = new Class.create();
CreditCardFieldsToggler.prototype = {
    initialize: function()
    {
        Event.observe('p_method_paypal', 'click', function() {
            var credit_card_fields = ["credit_card_fields_section"];

            for(var i = 0; i < credit_card_fields.length; i++)
            {
                document.getElementById(credit_card_fields[i]).style.display = 'none';
            }
        });

        Event.observe('p_method_authorizenet', 'click', function() {
            var credit_card_fields = ["credit_card_fields_section"];

            for(var i = 0; i < credit_card_fields.length; i++)
            {
                document.getElementById(credit_card_fields[i]).style.display = 'block';
            }
        });
    }
};
...
function showloginbox(ajaxLoginurl) {
    showLogin(ajaxLoginurl);
}
function showforgotpwd(url) {
    showpwdbox(url);
}
...
function retrievePassword(retrievePasswordUrl) {
    var passwordValidator = new Validation('osc-forgotpassword-form');
    if (passwordValidator.validate()) {
        showPassLoading();
        var url = retrievePasswordUrl;
