    ...
    public function getPosts()
   	{
		...
		foreach ($collection as $item) 
		{
			...
			/* fix for blog route problem */
			$t = rtrim($this->getUrl($item->getIdentifier()),"/");
			$r = (string)$t;
			...
			$item->setAddress(rtrim($this->getUrl($item->getIdentifier()),"/"));
			...
			if(Mage::getStoreConfig(AW_Blog_Helper_Config::XML_BLOG_USESHORTCONTENT) && strip_tags(trim($item->getShortContent()))){
				...
				//$content .= ' <a class="read_more" href="' . $this->getUrl($route . "/" . $item->getIdentifier()) . '" >'.$this->__('Read More').'</a>';
				/* fix for blog route problem */
				$content .= ' <a class="read_more" href="' . rtrim($this->getUrl($item->getIdentifier()),"/") . '" >'.$this->__('Read More').'</a>';
