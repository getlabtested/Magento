...
class Magestore_Onestepcheckout_Model_Observer extends Mage_Core_Controller_Varien_Action {
	...
	*/
    ...
    public function setQuoteItemsSingleQuantity($observer)
    {
        Mage::helper('onestepcheckout')->setAllQuoteItemsToSingleQty();
    }

    public function skipStraightToCheckoutIfNecessary($observer)
    {
        $skip_flag_is_set = Mage::getSingleton('core/session')->getSkipStraightToCheckoutOnCartAdd();

        if ($skip_flag_is_set)
        {
            Mage::getSingleton('checkout/session')->setNoCartRedirect(true);
            $response = $observer->getResponse();
            $onestepcheckout_route = Mage::helper('onestepcheckout')->getOnestepcheckoutRoute();

            $redirect_url = Mage::getUrl($onestepcheckout_route, array('_secure' => true));
