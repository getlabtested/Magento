...
class Magestore_Onestepcheckout_Block_Onestepcheckout extends Mage_Checkout_Block_Onepage_Abstract
{
    ...
    */
    ...
    {
        return Mage::getSingleton('checkout/session')->getTmpPost();
    }

    public function isTemporaryPost()
    {
        $temporary_post = $this->getTemporaryPost();
        return (is_array($temporary_post) && (count($temporary_post) > 0));
    }
    ...
    {
        return Mage::helper('onestepcheckout')->getLoginUrl();
    }

    public function getLoginPostUrl()
    {
        return Mage::helper('onestepcheckout')->getLoginPostUrl();
    }

    public function getForgotPasswordUrl()
    {
        return Mage::helper('onestepcheckout')->getForgotPasswordUrl();
    }

    public function getRetrievePasswordUrl()
    {
        return Mage::helper('onestepcheckout')->getRetrievePasswordUrl();
    }

    public function generateSessionInfoJson()
    {
        $session_info = array();
        $session_info['customer_is_logged_in'] = $this->isCustomerLoggedIn();
        $session_info['order_type'] = Mage::getSingleton('core/session')->getOrderType();
        $session_info['is_prescription'] = Mage::getSingleton('core/session')->getScript();

        $json_session_info = json_encode($session_info);
        return $json_session_info;
    }

    public function generateAjaxZipCodeJson()
    {
        $ajax_zipcode_info = array();
        $ajax_zipcode_info['ajax_enabled'] = (
                ($this->configData['enable_ajax'])
                && ($this->isAjaxBillingField('postcode'))
                && ($this->configData['show_zipcode'])
        );
        $ajax_zipcode_info['checkout_nnr_url'] =
            $this->getUrl('onestepcheckout/index/nnr', array('_secure' => true));

        $json_ajax_zipcode_info = json_encode($ajax_zipcode_info);

        return $json_ajax_zipcode_info;
    }
}
