...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    const ERROR_CUSTOMER_SESSION_INVALID_DOB_DATA = 'The dob data stored in the customer session was invalid: %s.';
    const ERROR_CORE_SESSION_INVALID_DOB_DATA = 'The dob data stored in the core session was invalid: %s.';
    ...
    public function setPPMDSpecificData($order, $is_paypal = false)
    {
        $post = Mage::app()->getRequest()->getPost();

        if ($is_paypal || ($post['payment']['method'] == 'authorizenet')){
        ...
        if (strlen(Mage::getSingleton('core/session')->getAffiliateId()) > 0 ) {
            $order->setAffiliateId(Mage::getSingleton('core/session')->getAffiliateId());
            $order->setPpmdAffiliate(Mage::getSingleton('core/session')->getAffiliateId());
        }
     ...
     * @return Mage_Checkout_Model_Type_Onepage
     */
    public function savePaypalOrder($order)
    {
        $this->validate();
        $isNewCustomer = false;
        switch ($this->getCheckoutMethod()) {
            ...
            case self::METHOD_REGISTER:
                ...
                $customer_post = Mage::getSingleton('customer/session')->getTmpPost();
                $email = $customer_post['billing']['email'];
                $this->getQuote()->getCustomer()->setEmail($email);
                $isNewCustomer = true;
                break;
            default:
        ...
        $this->setPPMDSpecificData($order, true);
        ...
        if ($order) {
            ...
            if (!Mage::getModel('core/session')->getScript()) {
                ...
                $customer = $this->getQuote()->getCustomer()
                    ->setGender($customer_post['billing']['gender'])
                    ->setPpmdPhone($customer_post['billing']['telephone'])
                    ->setPpmdNotify($customer_post['ppmd_notify']);

                $dob = $customer_post['m_select'].'/'.$customer_post['d_select'].'/'.$customer_post['y_select'];
                $testHelper = Mage::helper('ppmd_tests');
                if (!$testHelper->validateSlashBirthday($dob))
                {
                    $error_message = sprintf(self::ERROR_CUSTOMER_SESSION_INVALID_DOB_DATA, $dob);
                    Mage::log($error_message, Zend_Log::ERR, 'invalid_dob.log');
                    Mage::log($customer_post, Zend_Log::ERR, 'invalid_dob.log');
                    Mage::log($_SERVER['HTTP_USER_AGENT'], Zend_Log::ERR, 'invalid_dob.log');

                    // Attempt to retrieve the birthday from the core/session object
                    $core_customer_post = Mage::getSingleton('core/session')->getPostWithBirthday();
                    $dob_year = $core_customer_post['y_select'];
                    $dob_month = $core_customer_post['m_select'];
                    $dob_day = $core_customer_post['d_select'];
                    $core_dob = $dob_month.'/'.$dob_day.'/'.$dob_year;

                    if (!$testHelper->validateSlashBirthday($core_dob))
                    {
                        $error_message = sprintf(self::ERROR_CORE_SESSION_INVALID_DOB_DATA, $core_dob);
                        Mage::log($error_message, Zend_Log::ERR, 'invalid_dob.log');
                        Mage::log($core_customer_post, Zend_Log::ERR, 'invalid_dob.log');
                    }
                    else
                    {
                        $dob = $core_dob;
                ...
                $earr['order'] = $order->getData();
                $earr['customer'] = $customer->getData();
                $earr['post'] = $customer_post;
            ...
            if (Mage::getSingleton('core/session')->getScript()) {
                $data = Mage::getSingleton('core/session')->getScriptData();
                $cust = Mage::getModel('customer/customer')->load($order->getCustomerId());

                $scripts = implode(",", $data['prescriptions']);
                $scr = Mage::getModel('prescriptions/prescriptions');
                $scr->setOrderId($order->getId());
                $scr->setCustomerId($order->getCustomerId());
                $scr->setCustomerName($cust->getFirstname().' '.$cust->getLastname());
                $scr->setCustomerPhone($data['phone_1'].'-'.$data['phone_2'].'-'.$data['phone_3']);
                $scr->setPharmacyPhone($data['phone_pharmacy_1'].'-'.$data['phone_pharmacy_2'].'-'.$data['phone_pharmacy_3']);
                $scr->setAllergies($data['allergies']);
                $scr->setScripts($scripts);
                $scr->setCurMeds($data['prescriptions_medications']);
                $scr->setCurOtcMeds($data['prescriptions_medications']);
                $scr->setMedHistory($data['medical_history']);
                $scr->setStatus(0);
                $scr->setCreatedTime('NOW()');
                $scr->save();
            }
    ...
    public function saveOrder()
    {
        ...
        $this->setPPMDSpecificData($order, false);
        ...
        if ($order) {
            ...
            if (!Mage::getModel('core/session')->getScript()) {
                ...
                $customer = Mage::getModel('customer/customer')->load($custId)
                    ->setGender($post['billing']['gender'])
                    ->setDob($post['m_select'].'/'.$post['d_select'].'/'.$post['y_select'])
                    ->setPpmdPhone($post['billing']['telephone'])
                    ->setPpmdNotify($post['ppmd_notify']);
                $customer->save();
