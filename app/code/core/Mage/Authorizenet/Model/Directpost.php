...
class Mage_Authorizenet_Model_Directpost extends Mage_Paygate_Model_Authorizenet
{
    ...
    public function process(array $responseData)
    {
        ...
        Mage::getSingleton('admin/session')->setAuthResponseText($responseText);
        $isError = false;
        if ($orderIncrementId) {
    ...
    public function checkResponseCode()
    {
        switch ($this->getResponse()->getXResponseCode()) {
            ...
            case self::RESPONSE_CODE_DECLINED:
            case self::RESPONSE_CODE_ERROR:
            	Mage::getSingleton('admin/session')->setAuthResponse($this->_wrapGatewayError($this->getResponse()->getXResponseReasonText()));
