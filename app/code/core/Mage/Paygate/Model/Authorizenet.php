...
class Mage_Paygate_Model_Authorizenet extends Mage_Payment_Model_Method_Cc
{
    ...
    protected function _place($payment, $amount, $requestType)
    {
		...
		// Mage::Log('486');
		// Mage::Log(print_r($request));
// 		
		// Mage::Log('489');
		// Mage::Log(print_r($result));
    ...
    protected function _buildRequest(Varien_Object $payment)
    {
		...
		$p = Mage::app()->getRequest()->getPost('payment');
		
		$request->setXCardNum($p['cc_number'])
                ->setXExpDate(sprintf('%02d-%04d', $p['cc_exp_month'], $p['cc_exp_year']))
                ->setXCardCode($payment->getCcCid());
