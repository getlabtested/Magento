...
class Mage_Adminhtml_Model_Sales_Order_Create extends Varien_Object
{
     ...
     * @return  boolean
     */
    protected function _customerIsInStore($store, $customer = null)
    {
        $customer = is_null($customer) ? $this->getSession()->getCustomer() : $customer;
    ...
    protected function _loadCustomerByEmail($customer, $store)
    {
        $website_id = $store->getWebsiteId();
        $potential_existing_customer = Mage::getModel('customer/customer')->setWebsiteId($website_id);
        $customer_email = $this->_getNewCustomerEmail($customer);
        $potential_existing_customer->loadByEmail($customer_email);
        return ($potential_existing_customer->getId()) ? $potential_existing_customer : $customer;
    }
    ...
    public function _prepareCustomer()
    {
        ...
        $customer = $this->_loadCustomerByEmail($customer, $store);

        $customerIsInStore  = $this->_customerIsInStore($store, $customer);
        $billingAddress     = null;
    ...
    public function createOrder()
    {
		...
		foreach ($quote->getAllItems() as $item) {
			...
			if (in_array($item->getProductId(),array(34))) {
				
				$quote->setProductLine(2)->save();
    ...
    protected function _validate()
    {
        ...
        $this->getQuote()->getShippingAddress()->setShippingMethod('freeshipping');
