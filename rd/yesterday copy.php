...
$data = $api->data($idg, '', 'ga:visits,ga:pageviews,ga:newVisits,ga:visitors', false, $yesterdayGa, $yesterdayGa, 10, 1, $filters);
$visitorCount = $data['ga:visitors'];

$tv = $visitorCount;

$phone = getCalls($yesterday,$date);

$ref = getRefunds($yesterday,$date);
	
$sales = makeReport($yesterday,$date);
...
*/
  ...
  ?>
		...
		<td>$<?php echo round($sales['phone_totals']['authorizenet']->getTotal());?></td>
      	<td>$<?php echo round($sales['online_totals']['authorizenet']->getTotal());?></td>
      	
      	<td>$<?php echo round($scripts->getTotal());?></td>
      	
      	<td>$<?php echo round($sales['phone_totals']['paylater']->getTotal());?></td>
