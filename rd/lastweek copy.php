...
$data = $api->data($idg, '', 'ga:visits,ga:pageviews,ga:newVisits,ga:visitors', false, $lastSundayGa, $thisWeekGa, 10000, 1, $filters);
$visitorCount = $data['ga:visitors'];

$tv = $visitorCount;

$phone = getCalls($lastSunday,$thisWeek);
	
$sales = makeReport($lastSunday,$thisWeek);

$ref = getRefunds($lastSunday,$thisWeek);
	...
	$query = 'select count(*) as sales,
		...
		WHERE 
			...
			and a.created_at < "'.$thisWeek.'"
			and a.created_at > "'.$lastSunday.'";';    
...
$query = 'select SUM(a.lab_revenue) as total 
		...
		WHERE 
			a.ppmd_provider != 1  
			and b.updated_at < "'.$thisWeek.'" 
			and b.updated_at > "'.$lastSunday.'";';           
...
$data = $api->data($idg, '', 'ga:visits,ga:pageviews,ga:newVisits,ga:visitors', false, $twoSundaysAgoGa, $lastSundayGa, 10000, 1, $filters);
$visitorCount = $data['ga:visitors'];

$tv = $visitorCount;

$phone = getCalls($twoSundaysAgo,$lastSunday);
	
$sales = makeReport($twoSundaysAgo,$lastSunday);

$ref = getRefunds($twoSundaysAgo,$lastSunday);
	...
	$query = 'select count(*) as sales,
		...
		WHERE 
			...
			and a.status = "complete" 
			and a.created_at < "'.$lastSunday.'"
			and a.created_at > "'.$twoSundaysAgo.'";';    
...
$query = 'select SUM(a.lab_revenue) as total 
		...
		WHERE 
			a.ppmd_provider != 1  
			and b.updated_at < "'.$lastSunday.'" 
			and b.updated_at > "'.$twoSundaysAgo.'";';           
...
*/
  ...
  ?>
      	...
      	$tr = $sales['phone_totals']['authorizenet']->getRefunded()
      		+$sales['phone_totals']['paylater']->getRefunded()
      		+$sales['phone_totals']['paynearme']->getRefunded()
      		+$sales['online_totals']['authorizenet']->getRefunded()
      		+$sales['online_totals']['paynearme']->getRefunded();
